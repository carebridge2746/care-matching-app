...
├── lib/          # 라우트 매핑, 입력 검증, 날짜 처리, 공통 선택지 목록, 저장소 헬퍼
...
- Phase 6 (매칭): 간병인에게 요청 원문을 그대로 열지 않고, 매칭 계산을 Edge Function에서
  하거나 식별 정보를 뺀 뷰만 공개합니다.
- Phase 7 (매칭 성사): `matches` 테이블을 만들고, 배정된 간병인만 해당 요청과 환자 정보를
  볼 수 있는 정책을 추가합니다.
