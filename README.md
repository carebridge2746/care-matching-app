...
```bash
npm test          # 규칙·Mock 흐름·화면 부품 테스트 (jest-expo)
npm run lint
npx tsc --noEmit
```

데이터베이스 규칙 검사는 앱 테스트와 따로, Supabase SQL Editor 에서 `supabase/tests/` 의 스크립트로
돌립니다. 방법은 `supabase/tests/README.md` 에 있습니다.
...
│                 #   AI 결과로 요청 폼 채우기, 한국어 조사, 저장소 헬퍼
├── store/        # Zustand 상태 저장소
├── theme/        # 디자인 토큰 (색상, 타이포, 여백, 상태 색상)
└── types/        # 도메인 타입

__tests__/        # jest-expo 테스트 — lib(규칙) · api(Mock 흐름) · components(화면 부품)
test-utils/       # 테스트가 함께 쓰는 시연 계정과 자료 만들기
```

```
supabase/
├── schema.sql    # 데이터베이스 스키마 (테이블 · RLS 정책 · 함수 · 트리거)
├── tests/        # SQL Editor 에서 돌리는 규칙 검사 스크립트 (begin … rollback)
...
수료는 보호자의 추천 카드에 보이고, 적합도 점수에 **5점**(두 과정에서 만점)으로 작게 들어갑니다.
크게 넣으면 교육을 아직 못 들은 새 간병인이 계속 아래로 밀려 첫 매칭을 잡지 못하기 때문입니다 —
자세한 내용은 아래 '남은 일 정리' 절에 있습니다. Supabase 쪽 `recommendation_candidates()` 는 아직
수료를 내보내지 않아, 그 모드에서는 이 항목이 0점입니다.
...
보호자도 받은 후기 화면(`/guardian/reviews`)에서 간병인이 남긴 후기를 보고 신고할 수 있습니다.

Mock 모드에서는 `admin@care.test` 로 로그인해 확인할 수 있습니다.

## 전체 테스트와 UI 개선 (Phase 12)

### 테스트

Phase 9~11 에서는 규칙을 임시 스크립트로 확인하고 버렸습니다. 다시 돌릴 수 없는 확인은
다음 변경에서 아무것도 지켜 주지 못하므로, Expo 문서대로 `jest-expo` 를 붙여 저장소에 남겼습니다.

| 묶음 | 확인하는 것 |
| --- | --- |
| `__tests__/lib` | 매칭 점수·가능 시간·입력 검증·날짜·이름 가리기·노쇼 판정·조사·AI 결과로 폼 채우기 |
| `__tests__/api` | Mock 어댑터의 흐름 — 인증, 환자·요청·수락·진행·취소, 추천, 노쇼, 후기·신고·관리자 조치, 교육 채점, AI 요청 정리 |
| `__tests__/seed-consistency` | 교육 문항이 SQL 시드와 Mock 사본에서 글자까지 같은지 |
| `__tests__/components` | 신고 폼, 관리자 사유 폼, 후기 목록, 간병 카드 |
| `supabase/tests` | 같은 규칙을 데이터베이스 함수와 정책에서 — SQL Editor 에서 실행 |

Mock 어댑터를 테스트하는 이유는 화면이 보는 규칙의 기준 구현이기 때문입니다. Supabase 쪽은 같은
규칙을 `supabase/tests` 가 확인하고, 두 쪽이 같은 답을 내는지는 같은 항목을 양쪽에 두는 것으로 지킵니다.

테스트 패키지는 버전을 고정했습니다. `jest-expo` 57.0.5 와 최신 `react-test-renderer` 는
`react-native` 0.86.2 · `react` 19.2.3 과 짝이 맞지 않아 설치가 거절됩니다.

### 화면을 직접 따라가 보고 고친 것

웹에서 보호자 → 간병인 → 관리자 흐름을 처음부터 끝까지 눌러 보고 고쳤습니다.

| 문제 | 고친 것 |
| --- | --- |
| 간병 카드 전체가 버튼인데 그 안에 시작·종료 버튼이 또 있었다 (웹에서 잘못된 HTML) | 카드는 누르는 영역이 아니고, 상세로 가는 길은 "요청 자세히 보기" 링크로 |
| "9월 14일부터"인 간병을 13일에 시작하고 종료할 수 있었다 | 시작일(한국 시간)부터 시작 — `start_care()`(schema.sql 64)와 Mock 이 함께 막고, 카드는 언제부터 누를 수 있는지 알려 준다 |
| 간병 종료가 확인 없이 바로 됐다 | 되돌릴 수 없으므로 한 번 더 묻는다 |
| AI 카드가 값을 채워 놓고 "대부분 비워 두었습니다"라고 했고, 아래 카드를 "위"라고 불렀다 | 문구를 고치고 성별 선호도 보여 준다 |
| 신고 폼이 프로필 수정 화면 안에 열려 아래 버튼이 "프로필 저장"이었다 | 받은 후기 화면(`/caregiver/reviews`)을 따로 두었다 |
| 원문에 적은 지역·시간·예산을 아래 칸에 다시 적어야 했다 | "적은 내용으로 아래 조건 채우기" — 빈 칸만 채우고 고른 값은 덮어쓰지 않는다. 등록할 때 같은 원문을 다시 정리하지 않는다 |
| 한국어가 낱말 중간에서 줄이 바뀌었다("드립니/다") | 웹에서 `word-break: keep-all` |
| 오류 문구가 "이름을(를)"이었다 | 받침을 보고 조사를 하나만 붙인다(`lib/korean.ts`) |
| 긴 폼 끝에서 저장을 누르면 오류가 화면 밖에만 떴다 | 저장 버튼 위에 한 줄로 알린다 |
| 인지 상태가 '문제 없음'으로 미리 골라져 있었다 | 거동 상태처럼 직접 고르게 했다 |
| 후기 없는 간병인이 빈 별 다섯 개로 보여 0점처럼 읽혔다 | 후기가 없으면 별을 그리지 않는다 |
| 브라우저 탭 제목이 비어 있었다 | `expo-router/head` 로 "AI 간병 매칭" |
| 첫 화면의 관리자 설명이 실제 기능과 달랐다 | "신고된 후기와 문제가 생긴 매칭을 확인하고 처리합니다" |
| 환자가 한 명뿐인데도 요청마다 직접 골라야 했다 | 한 명이면 미리 골라 둔다 |

### 기록과 개인정보 규칙

점검에서 "누가 무엇을 지우고 볼 수 있는가"에 구멍이 넷 나왔습니다. 규칙을 정한 뒤 앱(Mock)과
데이터베이스(schema.sql 65~69)에 같은 답이 나오도록 넣었습니다.

| 문제 | 정한 규칙 | 지키는 곳 |
| --- | --- | --- |
| 노쇼 뒤 대기중으로 돌아간 요청을 지우면 노쇼·후기 기록이 cascade 로 사라졌다 | 매칭이 한 번이라도 붙은 요청은 지울 수 없고 취소만 된다 | 요청 삭제 정책 · `careRequestsApi.remove` |
| 환자를 지우면 그 환자의 모든 요청·매칭·후기가 사라졌다 | 기록이 없으면 행째 지우고, 있으면 **이름·관계·질환·특이사항만 지워 익명화**한다. 진행 중인 간병이 있으면 지우지 않는다 | `remove_patient()` · `patientsApi.remove` |
| 수락 전 간병인에게 보호자가 적은 원문이 그대로 보였다 | 원문은 수락한 간병인에게만 내려간다. 수락 전에는 고른 조건과 환자 요약으로 판단한다 | `caregiver_care_requests` 뷰 |
| 끝난 간병의 상대 연락처가 기한 없이 보였다 | 종료 **30일** 뒤 연락처·환자 특이사항·원문을 다시 가린다 | `match_details` 뷰 · `ContactRetentionDays` |
| 보호자가 요청의 `status`·조건을 직접 바꿀 수 있었다 | 요청 update 권한을 모두 거두고, 취소는 `cancel_care_request()` 로만 한다 | 컬럼 권한 · 함수 |

환자를 행째 지우지 않고 익명화하는 것은, 삭제를 막으면 보호자가 가족의 건강 정보를 지울 길이 없어지고,
그대로 지우면 간병인이 받은 후기와 노쇼 기록이 환자 한 명을 지우는 것으로 없어지기 때문입니다.
출생연도·성별·거동·인지 상태는 필수 칸이라 남기며, 이름이 지워지면 그것만으로 사람을 되짚기 어렵습니다.

30일은 마무리 연락과 재의뢰를 할 여유입니다. 앱의 `src/lib/privacy.ts` 와 뷰의 `interval '30 days'` 가
같은 값이어야 합니다.

### 실제 데이터베이스에서 확인한 것

빈 Supabase 프로젝트(PostgreSQL 17.6)에 `schema.sql` 을 적용하고 `supabase/tests` 01~04 를 돌려
검사 103개가 모두 통과했습니다. 그 과정에서 Mock 테스트로는 드러나지 않던 두 가지를 고쳤습니다.

- `create_review()` 와 `report_review()` 가 실제로는 한 번도 돌지 못했다. 인자 이름(`match_id`, `rating`,
  `reason` …)이 테이블 컬럼 이름과 같아 `on conflict` 가 붙은 insert 에서 "column reference is ambiguous"
  로 멈췄다. 인자를 함수 이름으로 한정하고 충돌 대상은 제약 이름으로 적었다.
- `schema.sql` 을 이미 적용된 데이터베이스에 다시 실행할 수 없었다. 단계마다 반환 모양이 넓어지는
  `recommendation_candidates()` 와 `match_details` 의 첫 정의 앞에 `drop … if exists` 를 두어,
  같은 파일을 두 번 적용해도 오류 없이 끝나는 것까지 확인했다.

### 남겨 둔 것
- 앱의 탈퇴는 기록을 남기도록 정했다(아래 '남은 일 정리'). 다만 데이터베이스에서 계정 행을 직접 지우면
  여전히 cascade 로 매칭이 함께 지워지므로, Supabase 쪽 탈퇴 함수를 붙일 때 함께 막는다.

## 안심 도착 (Phase 13)

보호자가 "간병인이 제시간에 오고 있는가"를 알 수 있게 합니다. **지금은 Mock 으로만 동작합니다** — 실제 GPS,
지도, 푸시 알림, Supabase 는 연결하지 않았고, 예상 도착 시각도 계산이 아니라 정해 둔 값입니다.

### 원칙

- **계속 추적하지 않습니다.** 간병 시작 **3시간 전**부터만 이동을 알릴 수 있고, 간병이 시작·종료·취소되면
  공유는 저절로 끝납니다. 끝난 간병의 공유는 다시 켤 수 없습니다.
- **간병인이 켜고 끕니다.** 안내를 읽고 동의해야 "이동 시작"이 나오며, 언제든 일시 중지하거나 끝낼 수 있습니다.
- **좌표를 보여 주지 않습니다.** 보호자에게는 상태 한 줄, 예상 도착, 마지막 업데이트만 보입니다.
- **위치로 노쇼를 정하지 않습니다.** 시작 시각이 지나도 도착이 확인되지 않으면 "확인 필요"로 알릴 뿐이고,
  노쇼는 지금처럼 보호자가 직접 신고해야 남습니다. 관리자 화면도 보기만 하고 제재·정산을 하지 않습니다.

### 상태

| 공유 상태 | 뜻 | 누가 바꾸는가 |
| --- | --- | --- |
| `notStarted` 이동 전 | 기록이 없거나 동의만 한 상태 | — |
| `sharing` 이동 중 | 이동 시작. Mock 예상 도착은 지금 + 40분 | 간병인 |
| `nearby` 약속 장소 인근 | Mock 예상 도착은 지금 + 5분 | 간병인 |
| `arrived` 도착 완료 | 이후 이동 버튼은 없다 | 간병인 |
| `paused` 일시 중지 | 보호자에게 "돌봄제공자가 위치 공유를 중지했습니다" | 간병인 (재개하면 멈추기 전 상태로) |
| `unavailable` 위치 확인 불가 | 이동 중인데 **20분** 넘게 업데이트가 없다 | 읽을 때 계산 |
| `ended` 공유 종료 | 간병인이 끝냈거나, 간병이 시작·종료·취소·노쇼가 됐다 | 간병인 · 매칭 상태 |

`unavailable` 과 매칭 상태로 끝난 `ended` 는 저장하지 않고 읽을 때 계산합니다(`lib/arrival.ts`의
`resolveLocationSharing`). 간병 시작·취소 코드마다 공유를 끄는 줄을 넣지 않아도 되고, 빠뜨릴 곳이 없습니다.

도착 판단(`assessArrival`)은 일정 전 · 출발 준비 · 이동 중 · **도착 지연 가능성 있음** · **확인 필요** ·
도착 완료 · 취소됨 가운데 하나입니다. 시작 시각을 적지 않은 요청은 오전 9시에 시작한다고 봅니다.

### 구조와 화면

화면 → `useLocationSharingStore` → `locationSharingApi` → Mock 어댑터(`location-sharing.mock.ts`) 입니다.
Supabase 어댑터는 자리만 있고 부르면 "아직 Mock 모드에서만 동작합니다"로 거절하며, 테이블·함수·뷰에 대한
TODO 를 적어 두었습니다.

| 화면 | 보이는 것 |
| --- | --- |
| 간병인 · 수락한 간병 | 카드 아래 안심 도착 — 동의 안내, 이동 시작 · 약속 장소 인근 · 도착 완료 · 일시 중지/재개 · 공유 종료 |
| 보호자 · 매칭 | 판단 배지, 상태 한 줄, 간병 시작 · 예상 도착 · 도착 · 마지막 업데이트, 할 수 있는 일 |
| 관리자 · 홈과 확인이 필요한 매칭 | 공유 시간대 안의 수락된 간병을 확인 필요 → 지연 가능성 순으로 (보기만) |

### 남겨 둔 것

- 실시간 갱신이 없어 보호자 화면은 다시 열 때 새 상태를 읽습니다.
- 실제 위치·경로로 예상 도착을 계산하지 않습니다. 붙일 때도 좌표는 서버에서만 쓰고 보호자에게는 상태만 내립니다.

## 남은 일 정리 — 시작 시각 · 교육 수료 · 탈퇴

Phase 13 뒤에 남겨 둔 일 가운데 외부 연결 없이 할 수 있는 것을 정리했습니다. **앱(Mock)에만 반영했고
`supabase/schema.sql` 은 고치지 않았습니다** — 두 쪽 규칙이 잠시 다르며, 맞춰야 할 곳은 코드에
`TODO(Supabase)` 로 적어 두었습니다.

### 시작 시각으로 판단합니다

| 동작 | 전 | 지금 |
| --- | --- | --- |
| 간병 시작 | 시작일이면 언제든 | 약속한 시작 **3시간 전**부터 (안심 도착과 같은 기준) |
| 노쇼 신고 | 시작일 당일부터 | 약속한 **시작 시각이 지난 뒤**부터 |

날짜만 보면 저녁 간병을 아침에 시작해 두거나, 아직 올 시각이 아닌데 "오지 않았다"고 신고할 수 있었습니다.
시각을 적지 않은 요청은 오전 9시로 보고, 화면에는 `09:00(시각 미정)` 으로 그 사실을 함께 보여 줍니다.
시작일 당일 시작 시각 전에는 보호자 카드에 신고 버튼 대신 언제부터 신고할 수 있는지 적습니다.
규칙은 `src/lib/arrival.ts`(`canStartCareAt` · `hasScheduledStartPassed`) 한 곳에 있고, 화면과 Mock 저장소가 함께 씁니다.

### 교육 수료를 보여 주고 점수에 조금 넣습니다

보호자의 추천 카드에 "앱 교육 수료 · 과정 이름"이 보이고, 적합도 점수에 **교육 수료** 항목(5점)이 생겼습니다.
두 과정을 수료하면 만점이고 그 이상은 벌어지지 않습니다. 합을 100으로 두려고 경력을 10→7점, 자격을 5→3점으로
줄였습니다 — 본인이 적는 자격보다 앱이 채점해서 확인한 수료를 조금 더 쳐 준 것입니다.
교육을 듣지 않은 사람도 다른 조건이 모두 맞으면 95점이라 새 간병인이 크게 밀리지 않습니다.
간병인 본인 화면(요청 목록·상세·홈)도 같은 점수가 나오도록 수료 기록을 함께 넘깁니다.

### 탈퇴 — 개인정보는 지우고 기록은 남깁니다

보호자·간병인 홈의 **계정** 화면에서 탈퇴할 수 있습니다. 한 번 더 묻고, 되돌릴 수 없습니다.

| 대상 | 탈퇴하면 |
| --- | --- |
| 계정 | 이름은 '탈퇴한 사용자'로 바꾸고 연락처는 지운다. 이메일을 비워 같은 주소로 새로 가입할 수 있고, 옛 계정으로는 로그인할 수 없다 |
| 예정·진행 중인 간병 | 있으면 탈퇴를 거절한다. 상대가 기다리는 간병을 두고 사라지면 연락할 길이 없다 |
| 보호자의 요청·환자 | 기록이 없으면 지우고, 있으면 요청 원문과 환자 이름·건강 정보를 지운다 (환자 삭제와 같은 규칙) |
| 간병인 프로필 | 자기소개를 지우고 추천 후보에서 뺀다 |
| 매칭·후기·노쇼·수료 기록 | 그대로 남긴다. 상대가 받은 평가가 한 사람의 탈퇴로 사라지면 안 된다 |

관리자 계정은 조치 기록의 주인이라 앱에서 탈퇴할 수 없습니다. Supabase 모드의 탈퇴는 아직 "Mock 모드에서만
동작합니다"로 거절합니다 — 계정 행을 앱에서 지우면 cascade 로 기록까지 사라지므로, 서비스 키를 가진 서버 함수로 붙여야 합니다.

### 문구

- 이동 중일 때 보호자 카드의 배지와 상태 문장이 둘 다 "이동 중"이던 것을, 문장은 "약속 장소로 이동하고 있습니다"로 바꿨습니다.
- 이름 가리기가 '삭제된 환자'를 '삭OOOO'처럼 사람 이름으로 가리던 것을, 자리 문구는 그대로 두게 했습니다.

### 아직 남은 일

외부 연결이나 사람의 판단이 필요해 이번에 하지 않은 것입니다.

- `schema.sql` 을 위 규칙에 맞추기 — `start_care()` · `report_no_show()` 의 시각 기준, `recommendation_candidates()` 의 수료, 탈퇴 함수
- 앱을 실제 Supabase로 돌리기, 안심 도착 저장소, 보호자 화면 실시간 갱신, 실제 위치로 예상 도착 계산
- AI 자연어 정리 실제 연결(Edge Function 배포와 API 키), 알림·결제
- 상용화 전 위치 공유 동의 문구와 개인정보 처리방침 법률 검토
...
| `patients` | 간병 대상자. 출생연도·성별·질환·거동/인지 상태·특이사항. 간병 기록이 있는 환자를 지우면 행은 남기고 익명화한 뒤 `deleted_at` 을 채운다 |
...
| 대상 | 지금 가진 권한 |
| --- | --- |
| 보호자 | 본인이 등록한 환자·요청만 조회/등록. 환자는 수정할 수 있고 요청은 수정할 수 없다(취소는 함수). 삭제는 매칭이 한 번도 붙지 않은 대기중 요청과, 간병 기록이 없는 환자만 — 기록이 있는 환자는 `remove_patient()` 가 익명화한다 |
...
| 창구 | 하는 일 |
| --- | --- |
| `public.caregiver_care_requests` (뷰) | 대기중 요청과 본인이 수락한 요청만, 보호자 정보 없이 조회. 요청 원문은 수락한 간병인에게만 |
| `public.cancel_care_request()` (함수) | 보호자가 요청을 취소한다. 살아 있던 매칭은 트리거가 함께 취소한다 |
| `public.remove_patient()` (함수) | 환자를 지우거나, 간병 기록이 있으면 익명화한다. 진행 중인 간병이 있으면 거절 |
...
| `public.match_details` (뷰) | 당사자가 서로와 간병 내용을 읽는다. 취소·노쇼 매칭과 종료 30일이 지난 매칭은 연락처·특이사항·원문을 다시 가린다 |
...
| 12 | 전체 테스트(jest-expo · SQL 검사 스크립트) 및 UI 개선 | 완료 |
| 13 | 안심 도착 — 이동·도착 상태 공유, 확인 필요 알림 (Mock) | 완료 |
| 13+ | 남은 일 정리 — 시작 시각 규칙 · 교육 수료 점수 · 탈퇴 (Mock) | 완료 |
