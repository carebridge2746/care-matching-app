...
아래는 규칙을 어떻게 둘지 먼저 정해야 해서 이번에 고치지 않았습니다.

- 노쇼 신고 뒤 대기중으로 돌아간 요청을 보호자가 지우면, 노쇼 기록이 함께 사라진다.
- 수락 전 간병인에게 보호자가 적은 요청 원문이 그대로 보인다. 원문에 이름이나 병원을 적으면 드러난다.
- 끝난 간병의 상대 연락처가 기한 없이 보인다.
- 보호자가 `care_requests.status` 를 직접 아무 값으로나 바꿀 수 있다(컬럼 권한).
